...
from CDPR.CDPR_depth_pipeline import CDPRPipeline
...
if "__main__" == __name__:
    ...
    # --config:指定训练配置文件（默认 train_cdpr_depth.yaml）
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_cdpr_depth.yaml",
    ...
    parser.add_argument(
        "--resume_run",
        action="store",
        default='output/train_cdpr_depth/checkpoint/latest',
    ...
    # --add_datetime_prefix: 是否给输出目录加时间前缀。
    parser.add_argument(
        "--add_datetime_prefix",
        action="store_true",
    ...
    # -------------------- Initialization --------------------
    # Resume previous run
    # 检查是否要继续上一检查点运行，通常是从头开始
    # cfg:读取自train_marigold.yaml
    ...
    # 记录当前训练的配置和代码版本，以便以后复现训练结果，只在新一轮训练中会运行
    ...
    # SLurm相关的集群数据copy代码，可忽略
    if is_on_slurm() and (not args.do_not_copy_data):
        # local scratch dir
        original_data_dir = base_data_dir
        base_data_dir = os.path.join(get_local_scratch_dir(), "CDPR_data")
        # copy data
