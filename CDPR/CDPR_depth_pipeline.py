...
from CDPR.ConfidencePredictor import SimpleUNet, CNNBackbone, ResNet18Backbone, MLPBackbone, TransformerBackbone


class CDPRDepthOutput(BaseOutput):
    """
...
class CDPRNormalOutput(BaseOutput):
...
class CDPRPipeline(DiffusionPipeline):
    """
    ...
    def __init__(
        ...
        if confidence_predictor is None:
            ...
            # 默认初始化
                ...
                confidence_predictor = TransformerBackbone(in_channels=8, out_channels=1)
    ...
    # 去噪步数设为yaml中的值：50
        ...
        Returns:
            `CDPRDepthOutput`: Output class for Marigold monocular depth prediction pipeline, including:
        ...
        pol_noise_std = getattr(self, "pol_noise_std", 0.0)
        # print(pol_noise_std)
        if pol_noise_std > 0:
            noise = torch.randn_like(pol_norm) * pol_noise_std
            pol_norm = pol_norm + noise
            pol_norm = torch.clamp(pol_norm, -1.0, 1.0)
        ...
        print(1111111111111111, pol_norm.min(), pol_norm.max())
        assert pol_norm.min() >= -1.00001 and pol_norm.max() <= 1.00001
        # ----------------- Predicting depth -----------------
        ...
        for batch in iterable:
            ...
            geo_pred_raw, alpha_raw = self.single_infer(
                rgb_in=batched_rgb,
                pol_in=batched_pol,
            ...
            alpha_pred_ls.append(alpha_raw.detach())
        alphas = torch.concat(alpha_pred_ls, dim=0)
        alphas = alphas.mean(dim=0)  # [1,h,w]
        alpha_np = alphas.squeeze().cpu().numpy()  # [h,w]
    ...
    def single_infer(
        ...
        # alpha = torch.zeros(B, 1, H, W, device=latent_cat.device, dtype=latent_cat.dtype)
        # alpha = torch.ones(B, 1, H, W, device=latent_cat.device, dtype=latent_cat.dtype)
        z_fused = rgb_latent + alpha * (pol_latent - rgb_latent) # [B, 4, h, w]
        # z_fused = self.encode_rgb(fusion)
        B, _, H, W = z_fused.shape
        # alpha = 0.5 * torch.ones(B, 1, H, W, device=z_fused.device, dtype=z_fused.dtype)
