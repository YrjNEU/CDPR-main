...
from CDPR import CDPRDepthPipeline
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Run single-image depth estimation using CDPR."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        ...
        # default="checkpoint/depth/CDPR-polCNNv1",
    ...
    pipe: CDPRDepthPipeline = CDPRDepthPipeline.from_pretrained(
        checkpoint_path, variant=variant, torch_dtype=dtype
    )
    # 初始化 confidence_predictor
    pipe.confidence_predictor = CNNBackbone(in_channels=8, out_channels=1)
    pipe.confidence_predictor.load_state_dict(torch.load('checkpoint/depth/CDPR-polCNN/confidence_predictor.pth'))
